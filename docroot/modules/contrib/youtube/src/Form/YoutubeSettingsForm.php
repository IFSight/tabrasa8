...
class YoutubeSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['text'] = [
      '#type' => 'markup',
      '#markup' => '<p>' . $this->t('The following settings will be used as default values on all YouTube video fields.  Many of these settings can be overridden on a per-field basis.') . '</p>',
    ];
    $form['youtube_global'] = [
      '#type' => 'fieldset',
    ...
    $form['youtube_global']['youtube_suggest'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Show suggested videos from any channel - as opposed to only the same channel - when playback ends (rel).'),
      '#default_value' => $config->get('youtube_suggest'),
    ];
    $form['youtube_global']['youtube_modestbranding'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Do not show YouTube logo on video player control bar (modestbranding).'),
      '#default_value' => $config->get('youtube_modestbranding'),
    ];
    $form['youtube_global']['youtube_theme'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Use a light colored control bar for video player controls (theme).'),
    ...
    $form['youtube_global']['youtube_color'] = [
      ...
      '#description' => $this->t('Note: the modestbranding parameter will be ignored when this is in use.'),
    ];
    $form['youtube_global']['youtube_enablejsapi'] = [
      ...
      '#description' => $this->t('For more information on the IFrame API and how to use it, see the <a href="@api_reference">IFrame API documentation</a>.',
    ...
    $form['youtube_global']['youtube_wmode'] = [
      ...
      '#description' => $this->t("Checking this will fix the issue of a YouTube video showing above a modal window (including Drupal's Overlay). This is needed if you have Overlay users in IE or have modal windows throughout your site."),
    ];
    $form['youtube_global']['youtube_override'] = [
      ...
      '#description' => $this->t('This will allow users to pass parameter values into the YouTube URL provided in the field input. For example, adding "&start=30" to the end of the URL would start the embedded video at the 30 second mark. This may have unintended side effects if parameter values are ever passed by accident.'),
    ];
    $form['youtube_thumbs'] = [
      '#type' => 'fieldset',
    ...
    $form['youtube_thumbs']['youtube_thumb_token_image_style'] = [
      ...
      '#description' => $this->t('Default image style for the output of a youtube_image_url token.'),
    ...
    $form['youtube_privacy'] = [
      ...
      '#description' => $this->t('Checking this box will prevent YouTube from setting cookies in your site visitors browser.'),
    ];
    $form['youtube_player_class'] = [
      '#type' => 'textfield',
      '#title' => $this->t('YouTube player class'),
      '#default_value' => $config->get('youtube_player_class'),
      '#description' => $this->t('The iframe of every player will be given this class. They will also be given IDs based off of this value.'),
    ];
