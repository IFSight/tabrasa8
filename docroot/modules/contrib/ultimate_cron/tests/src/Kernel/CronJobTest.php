...
class CronJobTest extends KernelTestBase {
  ...
  function testGeneratedJob() {
    ...
    $this->assertEqual(0, CronJob::load($values['id'])->loadLatestLogEntry()->start_time);

    // Check run counter, at this point there should be 0 runs.
    $this->assertEqual(0, \Drupal::state()->get('ultimate_cron.cron_run_counter'));

    // Run cron manually for the first time.
    \Drupal::service('cron')->run();

    // Check run counter, at this point there should be 1 run.
    $this->assertEqual(1, \Drupal::state()->get('ultimate_cron.cron_run_counter'));
    ...
    $this->assertNotEqual(0, $latest_log_entry);
    ...
    $this->assertEqual($log_entry_future, $cron_last_ran);
    $this->assertEqual(1, \Drupal::state()->get('ultimate_cron.cron_run_counter'));
    ...
    $this->assertNotEqual($log_entry_past, $latest_log_entry);
    $this->assertEqual(2, \Drupal::state()->get('ultimate_cron.cron_run_counter'));
  }
}
